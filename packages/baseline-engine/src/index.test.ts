...
describe('baseline-engine', () => {
  it('normalizes the verified SOVV BaselineRequest fields without exposing raw input to the model', async () => {
    ...
    expect(baseline.evidence.enduringTendencies[0]?.source).toBe('system_inference');
    expect(baseline.evidence.interpretiveSignals[0]?.confidence).toBe('unavailable');
  ...
  it('fails closed when birth-time certainty is unknown', async () => {
    ...
    expect(baseline.evidence.unknowns.some((item) => item.id === 'birth_time_unknown')).toBe(true);
  });
  ...
  it('returns reduced current-condition context without exact location', async () => {
    ...
    expect(current.evidence.currentConditions[0]?.source).toBe('system_inference');
  });
  ...
  it('returns an explicit unavailable state for unavailable current location', async () => {
    ...
    expect(current.evidence.currentConditions).toHaveLength(0);
  });
});
