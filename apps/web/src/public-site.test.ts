...
describe('Sovereign.OS launch surface', () => {
  it('explains the product, Baseline, and response sequence in direct language', () => {
    for (const phrase of [
      ...
      'A starting map—not a verdict',
      'Bring a real question. Leave with a clearer next move.',
      'WHAT I NOTICE',
      'LOOK INWARD',
      'ONE NEXT MOVE'
  ...
  it('explains privacy, permission, uncertainty, and visual authority clearly', () => {
    for (const phrase of [
      'Raw birth details',
      'separate decision for each requested use',
      'STILL UNKNOWN',
      'Private account export is not available at launch',
      'Example only. Your answer uses your permitted context'
    ]) expect(publicCopy).toContain(phrase);
  });
  ...
  it('keeps the landing focused while explaining the internal product', () => {
    ...
    for (const label of ['BASELINE DESIGN', 'WHAT USING IT FEELS LIKE', 'PEOPLE AND SYSTEMS', 'Privacy and control']) {
      expect(landing).toContain(label);
    }
    expect(landing).toContain('Live a life you’d choose to watch again.');
    expect(landing).not.toMatch(/See more clearly, from the start|Clearer decisions\. Better conversations/i);
  });

  it('ships an accessible, concrete response example rather than a decorative mockup', () => {
    for (const phrase of [
      'role="tablist"',
      'role="tabpanel"',
      'aria-live="polite"',
  ...
  it('uses responsive shared public design layers', () => {
    ...
    expect(launchCss).toContain('safe-area-inset-top');
    expect(launchCss).toContain('prefers-reduced-motion');
