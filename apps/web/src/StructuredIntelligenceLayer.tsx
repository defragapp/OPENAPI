...
export function installStructuredIntelligenceRuntime(): void {
  ...
  window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (response.ok && messageMatch && (init?.method ?? (input instanceof Request ? input.method : 'GET')).toUpperCase() === 'POST') {
  ...
  return (
    <>
      <button className={`structured-intelligence-trigger ${open ? 'is-open' : ''}`} onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-controls="structured-intelligence-panel">
        <span>S</span><strong>Structured view</strong>
      </button>
      <aside id="structured-intelligence-panel" className={`structured-intelligence-panel ${open ? 'is-open' : ''}`} aria-label="Validated Sovereign response structure" aria-hidden={!open}>
        ...
        <div className="structured-plan-badges">
          ...
          <span>{safetyLabel(plan.safety_mode)}</span>
          <span>{plan.response_phase === 'integration' ? 'Integrated response' : 'Clarifying phase'}</span>
        </div>

        <div className="structured-plan-scroll">
          <PlanSection label="WHAT SOVEREIGN NOTICED" value={plan.recognition} />
          {plan.response_phase === 'question' ? (
            <PlanSection label="LOOK INWARD" value={plan.inward_question} featured />
          ) : (
            <>
              <PlanSection label="WHAT THIS MAY BE SHOWING" value={plan.candidate_hidden_expectation || plan.recognition} />
              <PlanSection label="WHAT MAY BE PROTECTED" value={plan.protected_need} />
              <PlanSection label="A CLEARER FORM" value={plan.clearer_form} featured />
              <PlanSection label="PRACTICAL NEXT ACTION" value={plan.practical_action} />
            </>
          )}

          <section className="structured-basis">
            <header><p>BASIS</p><h3>What this response was allowed to use</h3></header>
            {basis.length ? <div>{basis.map((item) => <span key={item}>{item}</span>)}</div> : <p>No framework basis was displayed. The response remains exploratory.</p>}
          </section>

          <section className="structured-limits">
            <p>LIMITS</p>
            <h3>{limitTitle(plan)}</h3>
            <span>{limitCopy(plan)}</span>
          </section>

          {detail.covenantEnabled && (
            <section className="structured-covenant">
              <p>COVENANT IS ACTIVE</p>
              <h3>Scripture remains a separate lens.</h3>
              <span>Christian teaching and retrieved biblical references may support reflection, but they do not establish God’s exact intent or override consent, safety, responsibility, or the user’s judgment.</span>
            </section>
          )}
        </div>
      </aside>
...
function validPlan(value: unknown): value is SovereignPlan {
  ...
  return ['question', 'integration'].includes(String(plan.response_phase))
    && ['confirmed', 'supported', 'exploratory'].includes(String(plan.confidence))
    && ['standard', 'grounded', 'escalate'].includes(String(plan.safety_mode))
    && typeof plan.recognition === 'string'
    && typeof plan.inward_question === 'string'
    && typeof plan.candidate_hidden_expectation === 'string'
    && typeof plan.protected_need === 'string'
    && typeof plan.clearer_form === 'string'
    && typeof plan.practical_action === 'string'
    && Boolean(basis)
    && typeof basis?.user_confirmed === 'boolean'
    && ['human_design', 'gene_keys', 'astrology', 'relationship', 'live', 'numerology'].every((key) => Array.isArray(basis?.[key]));
}

function validSurface(value: unknown): value is string {
  return typeof value === 'string' && ['Today', 'Explore', 'People', 'Systems', 'Library', 'You'].includes(value);
}
...
function basisItems(basis: Basis): string[] {
  ...
  for (const [label, values] of [
    ['Human Design', basis.human_design],
    ['Gene Keys', basis.gene_keys],
    ['Astrology', basis.astrology],
    ['Relationship', basis.relationship],
    ['Current conditions', basis.live],
    ['Numerology', basis.numerology]
  ] as const) {
    for (const value of values.slice(0, 3)) output.push(`${label}: ${value}`);
  }
  return output.slice(0, 14);
}

function confidenceLabel(value: SovereignPlan['confidence']): string {
  if (value === 'confirmed') return 'Confirmed by the user';
  if (value === 'supported') return 'Supported by selected context';
  return 'Exploratory';
}

function safetyLabel(value: SovereignPlan['safety_mode']): string {
  if (value === 'escalate') return 'Safety support needed';
  if (value === 'grounded') return 'Grounded response';
  return 'Standard reflection';
}

function limitTitle(plan: SovereignPlan): string {
  if (plan.safety_mode === 'escalate') return 'This response should not carry the situation alone.';
  if (plan.safety_mode === 'grounded') return 'Only grounded, immediately supportable claims are shown.';
  if (plan.confidence === 'confirmed') return 'Confirmed experience is not the same as certainty about cause.';
  if (plan.confidence === 'supported') return 'The available context supports this view, but does not prove it.';
  return 'Treat this as a possibility to test against real experience.';
}

function limitCopy(plan: SovereignPlan): string {
  if (plan.safety_mode === 'escalate') return 'Use qualified or emergency support appropriate to the situation. Sovereign does not diagnose, investigate, or replace professional help.';
  return 'Sovereign does not know another person’s hidden motive, exact feeling, diagnosis, or future behavior. The user can confirm, correct, or reject the interpretation.';
}
