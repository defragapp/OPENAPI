...
export function AccountControlCenter() {
  ...
  async function refresh() {
    ...
    try {
      const [libraryData, deletionData] = await Promise.all([
        api('/api/v1/library'),
  ...
  async function rename(item: LibraryItem) {
    ...
    try {
      await api(`/api/v1/library/${encodeURIComponent(item.id)}`, {
        method: 'PATCH',
  ...
  async function requestDeletion() {
    ...
    try {
      const result = await api('/api/v1/deletion-jobs', {
        method: 'POST',
      ...
      {open && (
        <div className="account-control-backdrop" role="presentation" onMouseDown={(event) => {
          ...
          <section className="account-control-dialog" role="dialog" aria-modal="true" aria-labelledby="account-control-title">
            <header>
              <div><p>YOUR CONTROL</p><h2 id="account-control-title">Account & Library</h2></div>
              <button onClick={() => setOpen(false)} aria-label="Close account controls">×</button>
            </header>
            <p className="account-control-intro">Manage what you deliberately saved, open billing and permissions, or schedule account deletion without searching through separate product areas.</p>
            ...
            <nav className="account-control-links" aria-label="Account links">
              ...
              <a href="mailto:info@defrag.app">Support</a>
            </nav>
            ...
            <section className="account-control-section">
              ...
              {library.length === 0
                ? <p className="account-empty">Nothing saved yet.</p>
                : <div className="account-library-list">{library.map((item) => (
                    <article key={item.id}>
                      <label>Title<input value={edits[item.id] ?? ''} maxLength={120} onChange={(event) => setEdits((current) => ({ ...current, [item.id]: event.target.value }))} /></label>
                      <p>{item.body?.summary ?? item.summary ?? 'No summary is available.'}</p>
                      <small>{item.updatedAt ? `Updated ${new Date(item.updatedAt).toLocaleDateString()}` : 'Saved privately'}{item.body?.uncertainty ? ` · uncertainty ${item.body.uncertainty}` : ''}</small>
                      <div><button disabled={loading} onClick={() => void rename(item)}>Save title</button><button className="danger" disabled={loading} onClick={() => void remove(item)}>Delete</button></div>
                    </article>
                  ))}</div>}
            </section>
            ...
            <section className="account-control-section deletion-section">
              ...
              {deletionJob ? (
                <div className="active-deletion">
                  <strong>Deletion scheduled</strong>
                  <p>Status: {deletionJob.status}. Scheduled for {deletionJob.scheduledFor ? new Date(deletionJob.scheduledFor).toLocaleString() : 'the end of the grace period'}.</p>
                  <button disabled={loading || deletionJob.status !== 'grace'} onClick={() => void cancelDeletion()}>Cancel account deletion</button>
                </div>
              ) : (
                <div className="deletion-approval">
                  <label className="approval-check"><input type="checkbox" checked={deleteApproval} onChange={(event) => setDeleteApproval(event.target.checked)} /><span>I understand that deletion removes the account and its private data after the grace period, subject to required billing and legal retention.</span></label>
                  <label>Type DELETE to continue<input value={deletePhrase} onChange={(event) => setDeletePhrase(event.target.value)} autoComplete="off" /></label>
                  <button className="danger" disabled={loading || !deleteApproval || deletePhrase !== 'DELETE'} onClick={() => void requestDeletion()}>Schedule account deletion</button>
                </div>
              )}
            </section>
          </section>
