...
describe('production visual authority v1', () => {
  it('keeps the senior system global and the text-first demo authority last only for public product proof', () => {
  ...
  it('preserves founder visual foundations while the senior system stabilizes every page family', () => {
    for (const marker of [
  ...
  it('makes public product proof text-first rather than a workflow tutorial', () => {
    for (const marker of [
  ...
  it('reclaims the policy component and keeps non-demo pages governed by the senior system', () => {
  ...
  it('keeps all active authority styles structurally balanced', () => {
    for (const source of [productCohesion, intelligenceDemo, visualAuthority, seniorAuthority, demoV2]) {
