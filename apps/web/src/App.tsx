...
export function App() {
  ...
  const publicRoutes = ['/', '/product', '/pricing', '/about', '/login', '/signup', '/privacy', '/terms'];
  ...
  return (
    <div className="public-shell">
      ...
      <main className="public-main">
        {path === '/privacy' ? <LegalPage kind="privacy" /> : path === '/terms' ? <LegalPage kind="terms" /> : path === '/login' || path === '/signup' ? <AuthPublicPage mode={path.slice(1)} /> : path === '/pricing' ? <PricingPage /> : path === '/about' ? <AboutPage /> : path === '/product' ? <ProductPage /> : <HomePage />}
      </main>
      <footer className="public-footer"><p>No selling personal data. Stripe-hosted billing. Support payments are voluntary and do not grant entitlements.</p>{supportUrl ? <a href={supportUrl} rel="noreferrer" target="_blank">Support the work<span className="sr-only"> (opens Stripe in a new tab)</span></a> : <span>Support the work can be enabled with VITE_STRIPE_SUPPORT_URL.</span>}</footer>
    </div>
  );
}

function HomePage() {
  return <><section className="public-hero"><span className="eyebrow">ONE PRIVATE AI APPLICATION</span><h1>Sovereign.OS is a private Baseline environment.</h1><p>Understand your own patterns, current conditions, relationships, and systems without turning another person’s inner state into a claim.</p><div className="choice-row"><a className="button-link" href="/signup">Create a free account</a><a className="button-link secondary" href="/product">See how it works</a></div></section><section className="demo-grid" aria-label="Product demonstrations"><article><h2>Today</h2><p>Baseline tendency, current amplification, unknown actual state, and a simple Yes / Partly / Not today correction.</p></article><article><h2>Explore</h2><p>How I make decisions, how I communicate, how I learn, and what becomes stronger under pressure.</p></article><article><h2>People</h2><p>Consent-aware pair comparison with two plausible perspectives and no hidden-motive claims.</p></article><article><h2>Systems</h2><p>Family and team examples show roles, authority, handoffs, responsibilities, constraints, and shared objective.</p></article><article><h2>Library</h2><p>Explicitly save an understanding when you approve it. Conversations are not silently made permanent.</p></article></section></>;
}

function ProductPage() {
  return <section className="public-hero"><span className="eyebrow">PRODUCT</span><h1>One Sovereign workspace.</h1><p>Today, Explore, People, Systems, Library, and You are the authenticated surfaces. Defrag, Alignment, and Covenant remain internal lenses, not separate products.</p><div className="demo-grid"><article><h2>Baseline-first</h2><p>Useful before an incident: enduring tendencies, current amplification, observed behavior, and unknown state remain separate.</p></article><article><h2>Consent-aware</h2><p>People, systems, Library links, and Covenant context require deterministic server-side consent and entitlement checks.</p></article><article><h2>Private by design</h2><p>Raw birth inputs, exact private location, secrets, and hidden reasoning are not sent to Sovereign model context.</p></article></div></section>;
}
...
function AuthPublicPage({ mode }: { mode: string }) {
  return <section className="public-hero"><span className="eyebrow">PASSWORDLESS ACCOUNT</span><h1>{mode === 'login' ? 'Sign in by email.' : 'Create a private free account.'}</h1><p>Email verification, Turnstile protection, secure signed sessions, and Baseline onboarding are handled by the Cloudflare Worker. No payment is required to begin.</p><form className="auth-form"><input aria-label="Name" placeholder="Name" /><input aria-label="Email" placeholder="Email" type="email" /><label><input type="checkbox" /> I accept the Terms and Privacy Policy.</label><button type="button">Send magic link</button></form></section>;
}

function LegalPage({ kind }: { kind: 'privacy' | 'terms' }) {
  const privacy = kind === 'privacy';
  return <section className="legal-copy"><span className="eyebrow">HUMAN LEGAL REVIEW REQUIRED</span><h1>{privacy ? 'Privacy Policy' : 'Terms of Service'}</h1><p>{privacy ? 'Sovereign.OS processes account information, raw Baseline input, reduced Baseline representation, location precision, current conditions, invited people, relationship metadata, system membership, consent scopes, AI threads, saved understandings, billing identifiers, export and deletion jobs, operational logs, Cloudflare processing, AI Gateway processing, Stripe processing, email verification, Turnstile, retention, and consent revocation.' : 'Sovereign.OS is non-diagnostic and is not a medical, legal, crisis, or therapeutic replacement. These Terms cover accounts, verification, invited-person responsibilities, consent, acceptable use, Free and Sovereign+, subscriptions, cancellation, voluntary support payments, exports, deletion, availability, and contact.'}</p></section>;
}
