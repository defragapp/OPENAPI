...
export async function requestMagicLink(request: Request, env: Env, kind: 'signup' | 'login'): Promise<Response> {
  ...
  const recent = await env.DB.prepare("SELECT id FROM auth_magic_links WHERE email_normalized = ? AND created_at > datetime('now', '-2 minutes')")
    .bind(email)
    .first<{ id: string }>();
  const recentIp = await env.DB.prepare("SELECT COUNT(*) AS count FROM auth_magic_links WHERE requested_ip_hash = ? AND created_at > datetime('now', '-15 minutes')")
    .bind(ipHash)
    .first<{ count: number }>();
  ...
  try {
    ...
    await env.DB.prepare('DELETE FROM auth_magic_links WHERE id = ? AND used_at IS NULL').bind(id).run();
