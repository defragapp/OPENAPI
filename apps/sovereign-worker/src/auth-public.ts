...
export async function saveBaselineOnboarding(env: Env, accountId: string, input: { displayName?: string; birthDate?: string; birthTime?: string; birthTimeCertainty?: string; birthLocation?: string; currentLocationMode?: string }) {
  ...
  const reduced = {
    status: 'ready',
    source: env.SOVV_INTERNAL_BASE_URL ? 'sovv-contract-ready' : 'deterministic-local-reduced-context',
    summary: 'Baseline input was accepted and reduced before any model use.',
    privacy: 'Raw birth input and exact private location are stored separately from model context and never sent to Sovereign.'
  };
  await env.DB.prepare('INSERT OR REPLACE INTO baseline_onboarding (account_id, display_name, birth_date_hash, birth_time_certainty, birth_location_hash, current_location_mode, reduced_context_json, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
    .bind(accountId, displayName, await sha256Hex(input.birthDate), input.birthTimeCertainty || 'unknown', await sha256Hex(input.birthLocation), input.currentLocationMode || 'unavailable', JSON.stringify(reduced), 'ready').run();
  await env.DB.prepare('UPDATE persons SET display_name = ?, baseline_status = ?, updated_at = datetime(\'now\') WHERE account_id = ? AND role = ?')
    .bind(displayName, 'ready', accountId, 'self').run();
  return reduced;
}
