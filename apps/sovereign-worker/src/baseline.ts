...
async function computeViaPrivateBaseline(env: Env, input: BaselineInput) {
  if (env.BASELINE) return env.BASELINE.compute(input);
  return computeReducedBaseline(input);
}

export async function persistBaseline(env: Env, accountId: string, input: BaselineInput) {
  const computed = await computeViaPrivateBaseline(env, input);
...
export async function computeCurrentConditions(env: Env, accountId: string, mode: LocationPrecision) {
  if (env.BASELINE) return env.BASELINE.currentConditions({ accountId, locationPrecision: mode });
  return computeCurrentConditionsForService(env, accountId, mode);
}

export async function computeCurrentConditionsForService(env: Env, accountId: string, mode: LocationPrecision) {
