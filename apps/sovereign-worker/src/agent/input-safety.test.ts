...
describe('deterministic Sovereign input safety', () => {
  ...
  it('routes explicit imminent harm, dangerous ingestion, and immediate danger', () => {
    ...
    expect(decideSovereignInputSafety('I just swallowed a bottle of pills.').disposition).toBe('urgent');
    expect(decideSovereignInputSafety('I overdosed.').disposition).toBe('urgent');
  });
  ...
  it('does not escalate ordinary distress, prescribed medication, past events, or fictional examples', () => {
    for (const input of [
