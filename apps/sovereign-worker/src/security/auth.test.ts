...
describe('same-origin mutation protection', () => {
  it('requires an Origin header that matches the request origin', () => {
    expect(() => requireSameOrigin(new Request('https://app.test/api/v1/people', { headers: { origin: 'https://app.test' } }))).not.toThrow();
    expect(() => requireSameOrigin(new Request('https://app.test/api/v1/people'))).toThrow(Response);
    expect(() => requireSameOrigin(new Request('https://app.test/api/v1/people', { headers: { origin: 'https://evil.test' } }))).toThrow(Response);
  });
});
