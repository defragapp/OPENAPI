...
export async function requireAuth(request: Request, env: Env): Promise<AuthContext> {
  ...
  if (payload.sid) {
    const session = await env.DB.prepare("SELECT id FROM account_sessions WHERE id = ? AND revoked_at IS NULL AND expires_at > datetime('now')")
      .bind(payload.sid).first<{ id: string }>();
