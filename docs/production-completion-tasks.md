...
- The release build, tests, static-page verification, runtime security checks, and Stripe endpoint contracts passed.
- Remaining work is concentrated in user guidance, context integrity, account access, transactional communications, visual consistency, user controls, and discoverability.

## Active tranche — account access, communication, and first-use clarity

- [ ] Fix the live sign-in error path so the UI distinguishes verification, invalid email, rate limit, provider, and unexpected failures.
- [ ] Center and tightly contain Cloudflare Turnstile on login and signup at desktop and mobile sizes.
- [ ] Make Turnstile loading and failure states visible, accessible, and actionable.
- [ ] Preserve validated `returnTo` through link creation and redemption.
- [ ] Add premium branded HTML and plain-text templates for sign-in, signup, and relationship invitations.
- [ ] Keep all email links on approved Sovereign.OS hosts and include expiry, security, privacy, and support context.
- [ ] Replace pattern-heavy and generic authentication copy with approved Sovereign.OS language.
- [ ] Give signed-in users an unmistakable first action instead of presenting six equal destinations without guidance.
- [ ] Add tests for auth statuses, return routing, email rendering, Turnstile states, and first-use guidance.

## Product integrity — one Sovereign intelligence

- [ ] Keep Today, Explore, People, Systems, Library, and You as surfaces of one Sovereign intelligence, not separate products.
...
- [ ] Verify Resend/Cloudflare sender-domain alignment, SPF, DKIM, and DMARC.
- [ ] Use consistent Sovereign.OS sender name, reply-to, support address, logo, colors, and accessible email markup.
- [ ] Add branded templates for sign-in, invitation, permission change, billing, cancellation, and account deletion.
- [ ] Ensure forwarded/publicly visible messages do not expose tokens, private context, raw birth data, or internal identifiers.
- [ ] Add snapshot/content tests for text and HTML variants.
...
- [ ] Verify Turnstile hostname, action, token age, single-use behavior, and error observability.
- [ ] Preserve generic account-enumeration-safe responses while giving the current user useful local guidance.
- [ ] Verify magic-link expiry, replay prevention, session rotation, logout invalidation, CSRF assumptions, and return URL allowlisting.
...
- [ ] Test the complete public → signup → email → onboarding → Free and paid workspace flows.
