...
assert(reference.length > 6_500, 'Approved visual reference is missing, truncated, or unexpectedly small');
...
for (const profile of profiles) {
  ...
  const dom = await scrapeRenderedAudit(profile, captured.url, captured.content);
  if (sharp) {
  ...
  const referenceAuthority = profile.name.startsWith('desktop-') ? 'founder-reference' : 'structural-only';
  if (!comparison || comparison.skipped) {
    console.log('[visual-release] label=' + profile.name + ' status=skipped reason=sharp-unavailable');
  } else {
...
const report = {
  ...
  method: 'Cloudflare Browser Run full-page screenshots; founder-reference comparison for desktop and structural overflow, sequence, typography, and section-order verification for mobile',
  results
};
