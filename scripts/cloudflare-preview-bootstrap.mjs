...
const baselineWorkerDir = resolve(root, 'apps/baseline-worker');
...
const r2Name = process.env.PREVIEW_R2_NAME || 'sovereign-openapi-preview-artifacts';
const baselineWorkerName = process.env.PREVIEW_BASELINE_WORKER_NAME || 'sovereign-baseline-preview';
...
function sensitiveValues() {
  return [
    ...
    process.env.BASELINE_INTERNAL_TOKEN
...
function runBaseline(args, options = {}) {
  const result = spawnSync('pnpm', ['--filter', '@sovereign/baseline-worker', 'exec', 'wrangler', ...args], { cwd: root, encoding: 'utf8', stdio: options.capture ? ['ignore', 'pipe', 'pipe'] : 'inherit', env });
  if (result.status !== 0) {
    const detail = options.capture ? sanitize(result.stderr || result.stdout) : 'see Wrangler output';
    throw new Error(`baseline wrangler ${args.join(' ')} failed: ${detail}`);
  }
  return result.stdout ?? '';
}
...
function bucketRows(listJson) {
  return Array.isArray(listJson) ? listJson : (listJson.buckets ?? listJson.result ?? []);
}

function hasBucket(listJson) {
  return bucketRows(listJson).some((item) => item.name === r2Name || item.bucket_name === r2Name);
}

let createdBucket = false;
try {
  const buckets = parseJsonOutput(run(['r2', 'bucket', 'list', '--json'], { capture: true }));
  if (!hasBucket(buckets)) {
    run(['r2', 'bucket', 'create', r2Name], { capture: true });
    createdBucket = true;
  }
} catch (error) {
  throw new Error(`Unable to resolve preview R2 bucket ${r2Name}: ${sanitize(error instanceof Error ? error.message : String(error))}`);
}

const baselineConfigPath = resolve(baselineWorkerDir, '.wrangler.preview.generated.jsonc');
const configPath = resolve(workerDir, '.wrangler.preview.generated.jsonc');
try {
  const baselineConfig = JSON.parse(readFileSync(resolve(baselineWorkerDir, 'wrangler.jsonc'), 'utf8'));
  baselineConfig.name = baselineWorkerName;
  baselineConfig.env.preview.name = baselineWorkerName;
  baselineConfig.env.preview.vars = {
    ...baselineConfig.env.preview.vars,
    APP_ENV: 'preview',
    APP_VERSION: process.env.GITHUB_SHA || baselineConfig.env.preview.vars.APP_VERSION
  };
  writeFileSync(baselineConfigPath, JSON.stringify(baselineConfig, null, 2));
  const baselineSecrets = { BASELINE_INTERNAL_TOKEN: process.env.BASELINE_INTERNAL_TOKEN };
  for (const [name, value] of Object.entries(baselineSecrets)) {
    if (!value) continue;
    const result = spawnSync('pnpm', ['--filter', '@sovereign/baseline-worker', 'exec', 'wrangler', 'secret', 'put', name, '--env', 'preview', '--config', baselineConfigPath], { cwd: root, input: value, encoding: 'utf8', stdio: ['pipe', 'pipe', 'pipe'], env });
    if (result.status !== 0) throw new Error(`baseline wrangler secret put ${name} failed: ${sanitize(result.stderr || result.stdout)}`);
  }
  runBaseline(['deploy', '--env', 'preview', '--config', baselineConfigPath], { capture: true });
  ...
  const config = JSON.parse(readFileSync(resolve(workerDir, 'wrangler.jsonc'), 'utf8'));
	  ...
	  config.env.preview.r2_buckets = [{ binding: 'ARTIFACTS', bucket_name: r2Name }];
	  config.env.preview.services = [{ binding: 'BASELINE', service: baselineWorkerName }];
  config.env.preview.vars = {
    ...
    SOVV_INTERNAL_BASE_URL: process.env.SOVV_BASE_URL || '',
	    ...
	    STRIPE_PRICE_SOVEREIGN_PLUS_MONTHLY: process.env.STRIPE_PRICE_SOVEREIGN_PLUS_MONTHLY || process.env.STRIPE_PRICE_SOVEREIGN_PLUS || '',
	    STRIPE_PRICE_SOVEREIGN_PLUS_ANNUAL: process.env.STRIPE_PRICE_SOVEREIGN_PLUS_ANNUAL || '',
	    STRIPE_PRICE_SOVEREIGN_SUPPORT: process.env.STRIPE_PRICE_SOVEREIGN_SUPPORT || '',
  ...
  const secrets = {
    ...
    BASELINE_INTERNAL_TOKEN: process.env.BASELINE_INTERNAL_TOKEN,
  ...
  const deployedUrl = deployOutput.match(/https:\/\/[^\s]+\.workers\.dev/)?.[0] ?? `https://${workerName}.workers.dev`;
	  const metadata = { workerName, baselineWorkerName, d1Name, r2Name, createdDatabase, createdBucket, deployedUrl, databaseIdSource: 'cloudflare-api-or-wrangler', commitSha: process.env.GITHUB_SHA ?? 'local' };
  ...
  rmSync(baselineConfigPath, { force: true });
}
