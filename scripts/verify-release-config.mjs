...
function assertBinding(scope, name) {
  ...
  if (!scope?.services?.some((item) => item.binding === 'BASELINE')) throw new Error(`${name} missing private BASELINE service binding`);
}

assertBinding(config, 'default');
assertBinding(config.env?.preview, 'preview');
console.log('Release config verified queues=true consumers=true schedules=true r2=true baseline_service=true');
