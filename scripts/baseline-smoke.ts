...
async function main() {
  ...
  for (const forbidden of ['1990-05-17', '14:30', 'Austin', 'Paris', 'America/Chicago', 'Europe/Paris', 'latitude', 'longitude']) if (safe.includes(forbidden)) throw new Error(`raw private value leaked: ${forbidden}`);
  ...
  console.log('Baseline smoke passed raw_excluded=true timezone_excluded=true birthplace_external=false uncertainty_preserved=true unavailable_fails_closed=true fixture_test_only=true provenance=true sovv_commit=a3db94bccc75089723bef0cf5ff36c47064bd789');
}
