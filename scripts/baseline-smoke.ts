...
async function main() {
  const exact = await computeReducedBaseline({ birthDate: '1990-05-17', birthTime: '14:30', birthTimeCertainty: 'exact', birthplace: 'Austin, TX', locationPrecision: 'city_or_regional' }, { allowRecordedFixture: true });
  const unknown = await computeReducedBaseline({ birthDate: '1990-05-17', birthTimeCertainty: 'unknown', birthplace: 'Paris, France', locationPrecision: 'none' }, { allowRecordedFixture: true });
  ...
  const productionDefault = await computeReducedBaseline({ birthDate: '1990-05-17', birthTimeCertainty: 'unknown', birthplace: 'Paris, France', locationPrecision: 'none' });
  ...
  console.log('Baseline smoke passed raw_excluded=true uncertainty_preserved=true unavailable_fails_closed=true fixture_test_only=true provenance=true sovv_commit=a3db94bccc75089723bef0cf5ff36c47064bd789');
}
