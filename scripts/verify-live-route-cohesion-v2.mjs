...
const sansTypographyReplacement = "  assert(['-apple-system', 'SF Pro Display', 'Segoe UI', 'system-ui', 'Helvetica Neue'].some((family) => String(audit.typography.headingFamily).includes(family)), `${label}: heading is not using the native sans title stack (${audit.typography.headingFamily})`);";
