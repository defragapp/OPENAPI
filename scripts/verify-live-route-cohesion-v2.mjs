...
const sansTypographyReplacement = "  assert(['-apple-system', 'SF Pro Display', 'Segoe UI Variable Display', 'Segoe UI', 'system-ui', 'Helvetica Neue'].some((family) => String(audit.typography.headingFamily).includes(family)), `${label}: heading is not using the approved native SF/Segoe title stack (${audit.typography.headingFamily})`);";
...
if (/Optima|Candara/.test(sansTypographyReplacement)) {
  throw new Error('Route cohesion v2 still certifies the rejected legacy humanist title stack.');
}
