...
requireValue(rootConfig.main === 'apps/sovereign-worker/src/runtime-auth-entry.ts', 'Root config must use the active account and onboarding runtime');
...
for (const route of ['/forgot-password', '/reset-password', '/onboarding']) {
  requireValue(rootConfig.assets?.run_worker_first?.includes(route), `Production is missing account route ${route}`);
}

requireValue(workerConfig.main === 'src/runtime-entry.ts', 'Worker must use the stable local-development runtime entry');
...
requireValue(packageJson.scripts?.['verify:cloudflare-build']?.includes('verify:auth-funnel'), 'Canonical build must verify the account and onboarding funnel');
...
console.log('Direct Cloudflare release config verified existing_repo=true production_root=true modern_auth_entry=true cloudflare_builds_only=true github_actions=false isolated_preview=true fork=false r2=false queues=false d1=true durable_objects=true ai=true assets=true custom_domains=2 parent_routes=2 account_routes=3');
