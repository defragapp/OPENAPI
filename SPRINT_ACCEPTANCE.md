...
### Track ACCESS-1 — Stripe webhook Access policy (launch sprint)
- RESOLVED (definitive evidence): the Access application "Sovereign Stripe webhook" exists scoped to exactly `app.defrag.app/api/v1/stripe/webhook` with a single policy `decision=bypass` `include=[everyone]`. This is the correct minimal architecture — Access steps aside only for the exact webhook path so Stripe delivery is never blocked, while security is enforced entirely at the application layer. Confirmed live: unsigned POST to the webhook → 400; the dedicated Stripe-signature contract test is green. No Service Auth is needed; no change required.

### Track AUTH-2 — Live authenticated-journey failure paths (launch sprint)
- GREEN (production, real endpoints): signup without 18+ eligibility → HTTP 400 `{"status":"invalid","field":"eligibility"}`; invalid/expired redeem token → HTTP 400; unauthenticated `/api/v1/threads` and `/api/v1/you` → HTTP 401; malformed cross-origin signup → HTTP 403 (Origin enforcement). Valid `Origin` header returns the correct eligibility validation, proving CSRF/origin + eligibility gates are effective.
- BLOCKED (full journey via preview): the isolated preview Worker (`sovereign-openapi-preview…`) is protected by the account's Default-Deny Cloudflare Access. A correctly-scoped service-token allow app + valid service token were created (Access recognized the token — `service_token_status:true`), but Access still issued a login challenge on every attempt, indicating zone/account-level Access on the `sovereign-os-api.workers.dev` domain supersedes the account application (other worker apps here use email identity). Temporary infra was removed to restore the prior Default-Deny posture exactly. EXTERNAL ACTION REQUIRED: the account owner must grant the preview host a service-token or email allow at the Access layer (or provide a browser session) to enable live preview-journey verification. App-layer auth contracts remain GREEN via in-memory handler smokes (`smoke:auth`, `smoke:product`).

### Track BILL-2 — Stripe live flow (launch sprint)
