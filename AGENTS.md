...
- A technical release is complete only when both branded `/ready` endpoints report the exact target SHA, `ready: true`, migration `0019_deprecate_manual_capacity`, migration parity `current`, configured policy/privacy/capacity-reservation dependencies, and exact matching release evidence.
...
- Current candidate D1 schema is `0019_deprecate_manual_capacity`; deployed `0017_privacy_access_and_eligibility` remains immutable.
